...
def attackchar(maxbatch = None):
    ...
    for dataid, data in enumerate(test_loader):
        ...
        for k in range(inputs.size()[0]):
            print(f"\"{raw[k]}\"")
            print(f"\"{pred[k].item()}\"")
            with open('/home/ubuntu/user_lzl/train.txt','a') as file:
                file.write(f"\"{pred[k].item()}\",\"{raw[k]}\"\n")
            print(modified[k-inputs.size()[0]])
            print(pred2[k].item())
            with open('/home/ubuntu/user_lzl/test.txt','a') as file:
                file.write(f"\"{pred[k].item()}\",\"{modified[k-inputs.size()[0]]}\"\n")
