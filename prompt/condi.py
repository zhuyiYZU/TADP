...
dataset['validation'] = AgnewsProcessor().get_dev_examples("./datasets/TextClassification/agnews/")
dataset['test'] = AgnewsProcessor().get_test_examples("./datasets/TextClassification/agnews/")


# load a pretrained model, its token izer, its config, and its TokenzerWrapper by one function
...
mytemplate = PrefixTuningTemplate(model=plm,  tokenizer=tokenizer, text=' {"placeholder":"text_a"} {"special": "<eos>"} The keywords of the text is {"mask"} ', using_decoder_past_key_values=False)
...
train_dataloader = PromptDataLoader(dataset=dataset["train"], template=mytemplate, tokenizer=tokenizer,
    ...
    batch_size=5,shuffle=True, teacher_forcing=False, predict_eos_token=False, # be sure to pass predict_eos_token=True if your tempalte doesn't contain one, or you model may fail to stop generation.
